# Third-Party Components

This repository includes third-party designs and derivative works. Each item retains its original license unless explicitly stated otherwise.

---

## Booster Tube Brackets

* **Contributor:** Dom Lake
* **License:** Not specified (used with permission)
* **Notes:** No explicit license provided by contributor

**Files:**

* `Buck_Bracket_x1.stl`
* `Pump_Bracket_x2.stl`
* `Relay_Bracket_x1.stl`
* `Vape_Bracket_x1.stl`

---

## N-Filter Jewel Mount

* **Contributor:** Dom Lake
* **License:** Not specified (used with permission)

**Files:**

* `N-Filter Jewel Mount.stl`

---

## Cyclotron Riser

* **Contributor:** Dom Lake
* **License:** Not specified (used with permission)

**Files:**

* `Cake-NeoPixel-Bracket-Long-Legs.stl`
* `Cake-NeoPixel-Bracket-Short-Legs.stl`

---

## Top Speaker Bracket

* **Author:** BeezleBot
* **License:** Creative Commons Attribution 4.0 International (CC BY 4.0)
* **Source:** https://www.thingiverse.com/thing:5866066
* **Modifications:** Renamed file

**Files:**

* `Top_Speaker_Mount_Bracket.stl`

---

## Top Speaker Bracket Square

* **Based on:** Top Speaker Bracket by BeezleBot
* **License:** Creative Commons Attribution 4.0 International (CC BY 4.0)
* **Source:** https://www.thingiverse.com/thing:5866066
* **Modifications:** Modified dimensions, added mounting holes, renamed file

**Files:**

* `Top_Speaker_Mount_Bracket_Square.stl`

---

## Wand Connectors

* **Based on:** Spengler Wand Couplings by thedarkotaku88
* **Original License:** Creative Commons Attribution-NonCommercial 4.0 International (CC BY-NC 4.0)
* **Status:** Derivative work used with permission from the original author
* **Source:** https://www.thingiverse.com/thing:5866066
* **Modifications:** Resized, reworked mounting geometry and dimensions

> These files are **not licensed under GPL-3.0**.
> Redistribution and use must comply with the original CC BY-NC 4.0 license and any additional permissions granted by the author.

**Files:**

* `V2_Wand_Connector_Hole_Bottom.stl`
* `V2_Wand_Connector_Hole_Left.stl`
* `V2_Wand_Connector_Hole_Top.stl`
